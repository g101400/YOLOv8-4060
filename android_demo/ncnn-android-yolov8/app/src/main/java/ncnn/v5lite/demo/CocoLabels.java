package ncnn.v5lite.demo;

/**
 * COCO 80 类的中文名称表（与 jni/yolov5.cpp 中 class_names 顺序严格一一对应）。
 *
 * 为什么放在 Java 层：OpenCV 的 cv::putText 只有 ASCII 字形，画不出中文（会变成 ???），
 * 所以检测框标签改由 Java 层用系统字体绘制（Android 字体回退链含 Noto Sans CJK）。
 */
public final class CocoLabels
{
    private static final String[] CN = {
            "人", "自行车", "汽车", "摩托车", "飞机", "公交车", "火车", "卡车",
            "船", "红绿灯", "消防栓", "停止标志", "停车计时器", "长椅", "鸟", "猫",
            "狗", "马", "羊", "牛", "大象", "熊", "斑马", "长颈鹿",
            "背包", "雨伞", "手提包", "领带", "行李箱", "飞盘", "滑雪板", "单板滑雪",
            "运动球", "风筝", "棒球棒", "棒球手套", "滑板", "冲浪板", "网球拍", "瓶子",
            "酒杯", "杯子", "叉子", "刀", "勺子", "碗", "香蕉", "苹果",
            "三明治", "橙子", "西兰花", "胡萝卜", "热狗", "披萨", "甜甜圈", "蛋糕",
            "椅子", "沙发", "盆栽", "床", "餐桌", "马桶", "电视", "笔记本电脑",
            "鼠标", "遥控器", "键盘", "手机", "微波炉", "烤箱", "烤面包机", "水槽",
            "冰箱", "书", "时钟", "花瓶", "剪刀", "泰迪熊", "吹风机", "牙刷"
    };

    private CocoLabels() {}

    /** 类别索引 → 中文名；越界时回退为 "类别N"，避免崩溃。 */
    public static String cn(int label)
    {
        if (label >= 0 && label < CN.length)
            return CN[label];
        return "类别" + label;
    }

    public static int size()
    {
        return CN.length;
    }
}
